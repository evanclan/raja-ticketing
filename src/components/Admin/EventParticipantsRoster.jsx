...
export default function EventParticipantsRoster({ eventId, eventTitle, isOpen, onClose }) {
  ...
  const fetchParticipantsRoster = useCallback(async () => {
    ...
    try {
      ...
      const { data: rosterData, error: rosterError } = await supabase
        .rpc("get_event_participant_roster", { target_event_id: eventId });

      if (rosterError) {
        console.warn("Roster function not available, falling back to direct query");
      ...
      const { data: summaryData, error: summaryError } = await supabase
        .rpc("get_event_participant_summary", { target_event_id: eventId });
    ...
    const enhancedParticipants = await Promise.all(
      (participantsData || []).map(async (participant) => {
        ...
        if (participant.checked_in_by) {
          try {
            const { data: adminData } = await supabase.auth.admin.getUserById(participant.checked_in_by);
  ...
  const filteredParticipants = participants.filter(participant =>
    ...
    (participant.primary_participant && participant.primary_participant.toLowerCase().includes(searchTerm.toLowerCase()))
  );

  const exportToCSV = () => {
    const headers = ["Name", "Email", "Age", "Type", "Relationship", "Primary Participant", "Check-in Time", "Checked In By", "Method", "Notes"];
    const csvContent = [
      headers.join(","),
    ...
    link.download = `${eventTitle || 'Event'}_Participants_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  };
        ...
        <div style={{ 
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              ...
              <p style={{ color: "#6b7280", fontSize: "1rem", margin: 0 }}>
                {eventTitle || 'Event'} - Complete Attendance List
              </p>
            </div>
            <button
              onClick={onClose}
          ...
          {summary.total_attendees && (
            <div style={{ 
              ...
              <div style={{ textAlign: "center", padding: "0.75rem", backgroundColor: "white", borderRadius: "0.375rem" }}>
                <div style={{ fontSize: "1.5rem", fontWeight: "bold", color: "#3b82f6" }}>
                  {summary.total_attendees}
                </div>
                <div style={{ fontSize: "0.75rem", color: "#6b7280" }}>Total Attendees</div>
              </div>
              <div style={{ textAlign: "center", padding: "0.75rem", backgroundColor: "white", borderRadius: "0.375rem" }}>
              ...
              <div style={{ textAlign: "center", padding: "0.75rem", backgroundColor: "white", borderRadius: "0.375rem" }}>
                ...
                <div style={{ fontSize: "0.75rem", color: "#6b7280" }}>Family Members</div>
              </div>
              {summary.average_age && (
                <div style={{ textAlign: "center", padding: "0.75rem", backgroundColor: "white", borderRadius: "0.375rem" }}>
                  ...
                  <div style={{ fontSize: "0.75rem", color: "#6b7280" }}>Avg Age</div>
                </div>
              )}
            </div>
          )}
          ...
          {loading ? (
            ...
            <div style={{ 
              ...
              <div style={{ fontSize: "0.875rem" }}>
                {searchTerm ? "Try adjusting your search terms" : "Participants will appear here after they check in"}
              </div>
            </div>
          ) : (
                ...
                <tbody>
                  {filteredParticipants.map((participant, index) => (
                    <tr 
                      ...
                      style={{ 
                        borderBottom: "1px solid #e5e7eb",
                        backgroundColor: participant.participant_type === 'registered_user' ? "#f0f9ff" : "white"
                      }}
                    >
                      <td style={{ padding: "0.75rem" }}>
                        ...
                        {participant.participant_type === 'family_member' && participant.primary_participant !== 'N/A' && (
                      ...
                      <td style={{ padding: "0.75rem" }}>
                        <span style={{
                          ...
                          fontSize: "0.75rem",
                          fontWeight: "500",
                          backgroundColor: participant.participant_type === 'registered_user' ? "#dbeafe" : "#fef3c7",
                          color: participant.participant_type === 'registered_user' ? "#1e40af" : "#92400e"
                        }}>
                          {participant.participant_type === 'registered_user' ? '👤 User' : '👨‍👩‍👧‍👦 Family'}
                        </span>
                      </td>
                      <td style={{ padding: "0.75rem", fontSize: "0.875rem", color: "#6b7280" }}>
                        {participant.relationship !== 'N/A' ? participant.relationship : '-'}
                      </td>
                      <td style={{ padding: "0.75rem", fontSize: "0.875rem", color: "#6b7280" }}>
